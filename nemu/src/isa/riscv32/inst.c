...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */)         \
  ...
  */ 
  ...
  INSTPAT("?????? ?????? ????? 010 ????? 1110011",csrrs,I,int index = (imm>>12)&0xf,temp = csr_read(index);printf("csrrs,index:%d\n",index);csr_write(index,src1|temp);R(rd)=temp);
  INSTPAT("?????? ?????? ????? 001 ????? 1110011",csrrw,I,int index = (imm>>12)&0xf,temp = csr_read(index);printf("csrrs,index:%d\n",index);csr_write(index,src1);R(rd)=temp);
  /*
  I
  */ 
