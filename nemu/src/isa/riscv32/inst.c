...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */)         \
  ...
  */ 
  ...
  INSTPAT("000000 000000 00000 000 00000 1110011",ecall,I,uint32_t dnpc = isa_raise_intr(1,cpu.pc);/*printf("dnpc:%8x\n",dnpc);Mw(R(0),4,1);*/s->dnpc = dnpc);
  INSTPAT("?????? ?????? ????? 010 ????? 1110011",csrrs,I,int index = (imm)&0xf,temp = csr_read(index);printf("csrrs,index:%d\n",index);csr_write(index,src1|temp);R(rd)=temp);
  INSTPAT("?????? ?????? ????? 001 ????? 1110011",csrrw,I,int index = (imm)&0xf,temp = csr_read(index);printf("csrrs,index:%d\n",index);csr_write(index,src1);R(rd)=temp);
  /*
  I
  */ 
  ...
  */
  ...
  INSTPAT("0011000 00010 00000 000 00000 11100 11",mret,R,s->dnpc = csr_read(1));
