...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */)                   \
  ...
  */
  ...
  INSTPAT("??????? ????? ????? 000 ????? 01000 11", sb, S, flag = true; uint32_t i = s->isa.inst.val;  src = BITS(i, 24, 20);  Mw(src1 + imm, 1, src2));
  ...
  */ 
  ...
  INSTPAT("?????? ?????? ????? 010 ????? 0000011",lw,I, uint32_t temp = Mr(src1+imm,4);R(rd) = temp);
  ...
  INSTPAT("?????? ?????? ????? 010 ????? 1110011",csrrs,I,int index = (imm),temp = csr_read(index);csr_write(index,src1|temp);R(rd)=temp);
  INSTPAT("?????? ?????? ????? 001 ????? 1110011",csrrw,I,int index = (imm),temp = csr_read(index);csr_write(index,src1);R(rd)=temp);
  /*
  I
  */ 
  ...
  */
  ...
  S
  */
  INSTPAT("??????? ????? ????? 010 ????? 0100011",sw,S,flag = true; uint32_t i = s->isa.inst.val;  src = BITS(i, 24, 20);Mw(src1+imm,4,src2));
  INSTPAT("??????? ????? ????? 001 ????? 0100011",sh,S,flag = true; uint32_t i = s->isa.inst.val;  src = BITS(i, 24, 20);Mw(src1+imm, 2, src2));
  /*
  S
  */
  ...
  */
  ...
  INSTPAT("0011000 00010 00000 000 00000 11100 11",mret,R,s->dnpc = csr_read(0x341););
