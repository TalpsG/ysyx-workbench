...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */)         \
  ...
  */
  ...
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu, I, printf("lb,  pc:%8x, rd :%2d,addr:%8x\n",cpu.pc,rd,src1+imm);R(rd) = Mr(src1 + imm, 1));
  INSTPAT("??????? ????? ????? 000 ????? 01000 11", sb, S, printf("sb,  pc:%8x,data :%8x,addr:%8x\n",cpu.pc,src2,src1+imm);Mw(src1 + imm, 1, src2));
  ...
  */ 
  ...
  //load word
  INSTPAT("?????? ?????? ????? 001 ????? 0000011",lh,I,printf("lh,  pc:%8x, rd :%2d,addr:%8x\n",cpu.pc,rd,src1+imm);R(rd)=SEXT(Mr(src1+imm,2), 16));
  INSTPAT("?????? ?????? ????? 101 ????? 0000011",lhu,I,printf("lhu, pc:%8x, rd :%2d,addr:%8x\n",cpu.pc,rd,src1+imm);uint32_t temp =Mr(src1+imm,2);R(rd)=temp);
  ...
  INSTPAT("?????? ?????? ????? 010 ????? 0000011",lw,I,printf("lw,  pc:%8x, rd :%2d,addr:%8x\n",cpu.pc,rd,src1+imm); uint32_t temp = Mr(src1+imm,4);R(rd) = temp);
  INSTPAT("000000 0????? ????? 001 ????? 0010011",slli,I,R(rd)=src1<<shamt);
  INSTPAT("?????? ?????? ????? 000 ????? 0000011", lb, I,printf("lb,  pc:%8x, rd :%2d,addr:%8x\n",cpu.pc,rd,src1+imm);R(rd) =  SEXT(Mr(src1 + imm, 1),8));
  INSTPAT("?????? ?????? ????? 110 ????? 0010011",ori,I,R(rd)=src1|imm);
  INSTPAT("?????? ?????? ????? 010 ????? 0010011",slti,I,int32_t temp1 = src1,temp2 = imm;R(rd)=(temp1<temp2)?1:0);
  INSTPAT("000000 000000 00000 000 00000 1110011",ecall,I,isa_raise_intr(1,cpu.pc));
  INSTPAT("?????? ?????? ????? 010 ????? 1110011",csrrs,I,int index = (imm>>12)&0xf,temp = csr_read(index);csr_write(index,src1|temp);R(rd)=temp);
  INSTPAT("?????? ?????? ????? 001 ????? 1110011",csrrw,I,int index = (imm>>12)&0xf,temp = csr_read(index);csr_write(index,src1);R(rd)=temp);
  /*
  I
  */ 
  ...
  */
  ...
  S
  */
  INSTPAT("??????? ????? ????? 010 ????? 0100011",sw,S,printf("sw,  pc:%8x,data :%8x,addr:%8x\n",cpu.pc,src2,src1+imm);Mw(src1+imm,4,src2));
  INSTPAT("??????? ????? ????? 001 ????? 0100011",sh,S,printf("sh,  pc:%8x,data :%8x,addr:%8x\n",cpu.pc,src2,src1+imm);Mw(src1+imm, 2, src2));
  /*
  S
  */
  ...
  */
  ...
  INSTPAT("0100000 ????? ????? 000 ????? 0110011",sub,R,int32_t s1 = src1,s2 = src2,temp = s1-s2;R(rd)=temp);
  ...
  INSTPAT("0000001 ????? ????? 001 ????? 0110011",mulh,R,int32_t s1 = src1,s2 = src2;int64_t temp,s1_ext = s1,s2_ext = s2;temp = s1_ext*s2_ext;R(rd) = (temp>>32)&(0xffffffff));

  INSTPAT("0000000 ????? ????? 100 ????? 0110011",xor,R,R(rd)=src1^src2);
  INSTPAT("0000000 ????? ????? 110 ????? 0110011",or,R,R(rd)=src1|src2);
  INSTPAT("0000001 ????? ????? 011 ????? 0110011",mulhu,R,uint64_t temp,s1_ext = src1,s2_ext = src2;temp = s1_ext*s2_ext;R(rd) = (temp>>32)&(0xffffffff));
