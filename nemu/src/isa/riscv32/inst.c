...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */)         \
  ...
  */
  ...
  //I
  //*/ 
  INSTPAT("?????? ?????? ????? 000 ????? 0010011",addi,I,R(rd)=imm+src1);// used
  ...
  //R
  //*/
  INSTPAT("0000001 ????? ????? 000 ????? 0110011",mul,R,int32_t s1 = src1,s2 = src2;R(rd)=s1*s2);
  //// mul high 将寄存器的值符号扩展到64位，有符号数乘法，结果取高32位存入rd
  INSTPAT("0000001 ????? ????? 001 ????? 0110011",mulh,R,int64_t s1=src1,s2=src2;R(rd)=(s1*s2)>>32);
  ...
  INSTPAT("0000000 ????? ????? 000 ????? 0110011",add,R,int32_t s1 = src1,s2 = src2;R(rd)=s1+s2);
