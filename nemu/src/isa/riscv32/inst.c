...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */)         \
  ...
  */ 
  ...
  INSTPAT("?????? ?????? ????? 010 ????? 0000011",lw,I,uint32_t temp = Mr(src1+imm,4);printf("lw addr:%8x,data:%8x\n",src1+imm,temp);R(rd) = temp);
  ...
  */
  ...
  S
  */
  INSTPAT("??????? ????? ????? 010 ????? 0100011",sw,S,printf("sw addr:%8x,data:%8x\n",src1+imm,src2);Mw(src1+imm,4,src2));
